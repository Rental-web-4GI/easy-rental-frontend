    ...
    <div className="space-y-8 animate-in fade-in duration-700 pb-10 text-left">
      <div className="bg-white dark:bg-[#1a1d2d] p-10 rounded-[3rem] border-b-4 border-slate-200 dark:border-slate-800 shadow-sm relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h1 className="text-4xl font-[900] italic text-slate-900 dark:text-white tracking-tighter uppercase leading-none">Console Opérationnelle</h1>
          <p className="text-slate-400 mt-2 italic text-sm font-medium">Agence : <span className="text-[#0528d6] font-black uppercase">{agencyData?.name}</span></p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white dark:bg-[#1a1d2d] rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm">
          <h3 className="text-xs font-black uppercase italic tracking-widest flex items-center gap-2 mb-8 text-slate-400"><TrendingUp size={14}/> Croissance CA</h3>
        ...
        <div className="bg-white dark:bg-[#1a1d2d] rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800 shadow-sm">
          <h3 className="text-xs font-black uppercase italic tracking-widest flex items-center gap-2 mb-8 text-slate-400"><Activity size={14}/> Flux Locations</h3>
          <div className="flex items-end gap-1.5 h-32">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white dark:bg-[#1a1d2d] rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm p-10 grid md:grid-cols-2 gap-12">
            <div>
                <h3 className="font-black italic uppercase tracking-tighter flex items-center gap-3 mb-8 text-[#0528d6]"><PieChart size={18} /> Disponibilité Parc</h3>
                <div className="space-y-5">
                    {Object.entries(vehicleStatusDistribution.distribution).map(([status, count]: any) => (
                        <div key={status} className="flex justify-between items-center p-4 bg-slate-50 dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800">
                            <span className="text-[10px] font-black uppercase text-slate-500 italic">{status}</span>
            ...
            <div>
                <h3 className="font-black italic uppercase tracking-tighter flex items-center gap-3 mb-8 text-orange-500"><CheckCircle size={18} /> Statut des Dossiers</h3>
                <div className="space-y-5">
        ...
        <div className="space-y-6">
          <div className="bg-[#0528d6] rounded-[2.5rem] p-8 text-white shadow-xl relative overflow-hidden">
             <h4 className="font-bold mb-4 italic uppercase text-xs tracking-widest opacity-80">Indicateurs Temps Réel</h4>
             <div className="space-y-6">
                <div className="bg-white/10 p-4 rounded-2xl border border-white/10">
                    <p className="text-[9px] font-black uppercase opacity-60 mb-1">Locations Actives</p>
                    <p className="text-3xl font-black italic">{summary.activeRentals}</p>
                </div>
                <div className="bg-white/10 p-4 rounded-2xl border border-white/10">
                    <p className="text-[9px] font-black uppercase opacity-60 mb-1">Total Réservations</p>
                    <p className="text-3xl font-black italic">{summary.totalReservations}</p>
                </div>
             </div>
             <button className="mt-8 w-full flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-tighter bg-white text-[#0528d6] py-4 rounded-xl shadow-lg hover:scale-[1.02] transition-all italic">
