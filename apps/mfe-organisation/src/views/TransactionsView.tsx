...
export const TransactionsView = ({ orgData , t }: { orgData: any, t: any }) => {
  ...
  const totalIncome = transactions.filter(tr => tr.type === 'RENTAL_INCOME').reduce((acc, tr) => acc + Math.abs(tr.amount || 0), 0);
  const totalExpense = transactions.filter(tr => tr.type === 'SUBSCRIPTION_COST').reduce((acc, tr) => acc + Math.abs(tr.amount || 0), 0);
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 pb-10">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard label={t.transactions.statTotal} value={transactions.length} icon={<Banknote />} />
        <StatCard label={t.transactions.statIncome} value={`${totalIncome.toLocaleString()} XAF`} icon={<ArrowUpRight className="text-green-500"/>} />
        <StatCard label={t.transactions.statExpense} value={`${totalExpense.toLocaleString()} XAF`} icon={<ArrowDownRight className="text-red-500" />} />
      </div>
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white dark:bg-[#1a1d2d] p-4 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="flex bg-slate-100 dark:bg-slate-900 p-1.5 rounded-2xl w-full md:w-auto">
            {[
              ...
              { id: 'RENTAL_INCOME', label: t.transactions.tabIncome },
              { id: 'SUBSCRIPTION_COST', label: t.transactions.tabExpense }
            ].map(tab => (
