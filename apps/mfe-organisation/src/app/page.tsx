...
import { 
  ...
  Database, Plus, Download, MoreHorizontal, Globe, Languages, Sun, Moon, Menu, X
} from 'lucide-react';
  ...
  const SidebarContent = () => (
    <>
      <div className="flex items-center gap-3 mb-16 px-2">
        ...
        <div>
          <h1 className="text-xl font-[900] italic uppercase tracking-tighter leading-none dark:text-white">
        ...
        {[
          ...
          <button 
            ...
            className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest transition-all ${
              item.active 
              ? 'bg-blue-600 text-white shadow-xl shadow-blue-100 dark:shadow-none italic' 
              : 'text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/10'
            }`}
          >
  ...
  return (
    <div className="flex h-screen bg-[#F8F9FC] dark:bg-[#0f1323] text-slate-900 dark:text-white font-sans transition-colors duration-300 overflow-hidden">
        ...
        <div className="fixed inset-0 z-[150] lg:hidden">
          ...
          <aside className="absolute left-0 top-0 bottom-0 w-72 bg-white dark:bg-[#1a1d2d] p-8 flex flex-col animate-in slide-in-from-left duration-300">
            <div className="flex justify-end mb-4">
               <button onClick={() => setSidebarOpen(false)}><X size={24} className="text-slate-400"/></button>
            </div>
            <SidebarContent />
          </aside>
        </div>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* HEADER */}
        <header className="h-24 bg-white/80 dark:bg-[#1a1d2d]/80 backdrop-blur-md border-b border-slate-100 dark:border-slate-800 px-6 md:px-10 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-4">
            <button onClick={() => setSidebarOpen(true)} className="lg:hidden p-3 bg-slate-50 dark:bg-slate-800 rounded-xl">
              <Menu size={20} />
            </button>
            <h2 className="text-lg md:text-2xl font-[900] italic uppercase tracking-tighter">{t.header.title}</h2>
          </div>
          
          <div className="flex items-center gap-3 md:gap-6">
            <div className="relative hidden xl:block group">
              <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300" size={18}/>
              <input className="bg-slate-50 dark:bg-[#0f1323] rounded-2xl py-3 pl-12 pr-6 text-xs font-bold w-64 outline-none focus:ring-2 focus:ring-blue-600 transition-all" placeholder={t.header.search} />
            </div>
            
            <div className="flex items-center gap-2">
              <button onClick={handleInstall} className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl text-blue-600 dark:text-blue-400 border border-slate-100 dark:border-slate-700 hover:bg-blue-50">
                <Download size={18}/>
              </button>
              <button onClick={() => setLang(l => l === 'FR' ? 'EN' : 'FR')} className="p-3 bg-slate-50 dark:bg-slate-800 rounded-xl text-[10px] font-black uppercase border border-slate-100 dark:border-slate-700">{lang}</button>
            ...
            <div className="flex items-center gap-3 pl-4 md:pl-6 border-l border-slate-100 dark:border-slate-800">
              ...
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-2xl bg-slate-200 dark:bg-slate-700 border-2 md:border-4 border-white dark:border-slate-800 shadow-xl overflow-hidden shrink-0">
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-10 custom-scrollbar">
            ...
            {[
              ...
              <div key={i} className={`bg-white dark:bg-[#1a1d2d] p-8 rounded-[2.5rem] shadow-sm border ${kpi.highlight ? 'border-orange-500/20' : 'border-slate-100 dark:border-slate-800'} flex flex-col justify-between h-44 relative overflow-hidden group hover:shadow-xl transition-all`}>
                <div className="flex justify-between items-start">
                  ...
                  {kpi.grow && <span className="text-[10px] font-black text-green-600 bg-green-50 dark:bg-green-900/20 px-3 py-1 rounded-full">{kpi.grow}</span>}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 bg-white dark:bg-[#1a1d2d] p-6 md:p-10 rounded-[3rem] shadow-sm border border-slate-100 dark:border-slate-800">
               <div className="flex justify-between items-start mb-10">
                  <div>
                    <h3 className="text-xl md:text-2xl font-[900] italic uppercase tracking-tighter">{t.charts.title}</h3>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{t.charts.sub}</p>
                  </div>
                  <button className="px-4 py-2 bg-slate-50 dark:bg-slate-800 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-400">{t.charts.period}</button>
               </div>
            ...
            <div className="bg-[#0f1323] p-8 md:p-10 rounded-[3rem] shadow-2xl text-white flex flex-col justify-between relative overflow-hidden h-[400px] lg:h-auto">
               ...
               <button className="w-full py-4 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white hover:text-[#0f1323] transition-all mt-6">{t.system.log}</button>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-[#1a1d2d] rounded-[3.5rem] shadow-sm border border-slate-100 dark:border-slate-800 overflow-hidden">
            <div className="p-6 md:p-10 border-b border-slate-50 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
               <div>
                  <h3 className="text-2xl md:text-3xl font-[900] italic uppercase tracking-tighter">{t.table.title}</h3>
                  <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{t.table.sub}</p>
               </div>
               <div className="flex gap-3 w-full sm:w-auto">
                  <button className="flex-1 sm:flex-none px-6 py-4 bg-blue-600 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest italic shadow-xl shadow-blue-200 dark:shadow-none hover:scale-105 transition-all flex items-center justify-center gap-2"><Plus size={16}/> {t.table.add}</button>
                  <button className="px-4 py-4 bg-slate-50 dark:bg-slate-800 rounded-2xl text-slate-500 hover:bg-slate-100 transition-all flex items-center justify-center border border-slate-100 dark:border-slate-700"><Download size={16}/></button>
               </div>
            </div>
            <div className="overflow-x-auto">
               <div className="min-w-[800px] px-10 pb-10">
                  <table className="w-full text-left">
                    ...
                    <tbody className="divide-y divide-slate-50 dark:divide-slate-800">
                       {[
                         { name: "Downtown Hub", loc: "New York, NY", rentals: 142, rev: "$45,200", status: "Online", sCol: "bg-green-500" },
                         { name: "Westside Rentals", loc: "Los Angeles, CA", rentals: 98, rev: "$32,150", status: "Syncing", sCol: "bg-blue-500" },
                         { name: "Airport Logistics", loc: "Chicago, IL", rentals: 215, rev: "$65,800", status: "Online", sCol: "bg-green-500" },
                       ].map((item, i) => (
