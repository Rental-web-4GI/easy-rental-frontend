  ...
  return (
    <Portal>
      <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4">
        ...
        <div className="relative w-full max-w-2xl bg-white dark:bg-[#1a1d2d] rounded-[3.5rem] shadow-2xl overflow-hidden animate-in zoom-in duration-300 border border-white/20">
          <div className="px-10 py-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50">
            <div className="flex gap-2">
              {[1, 2, 3, 4].map(i => <div key={i} className={`h-1.5 rounded-full transition-all duration-500 ${step >= i ? 'bg-[#0528d6] w-12' : 'bg-slate-200 dark:bg-slate-700 w-4'}`} />)}
            </div>
            <button onClick={onClose} className="p-2.5 bg-white dark:bg-slate-800 rounded-full shadow-sm hover:text-red-500 transition-colors"><X size={20}/></button>
          </div>

          <div className="p-10 text-left">
            {error && <div className="mb-6 p-4 bg-red-50 dark:bg-red-900/20 border-2 border-red-100 dark:border-red-900/30 rounded-2xl flex items-center gap-3 text-red-600 dark:text-red-400 text-[10px] font-black uppercase italic"><AlertCircle size={18}/> {error}</div>}

            {step === 1 && (
              <div className="space-y-8 animate-in slide-in-from-right-4">
                <h3 className="text-3xl font-[900] italic tracking-tighter uppercase text-[#0528d6] leading-none">Configuration</h3>
                <div className="space-y-5">
                  <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-800 dark:text-slate-400 uppercase italic ml-1">N° Téléphone de Contact</label><input value={form.clientPhone} onChange={e => setForm({...form, clientPhone: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-2xl font-black text-sm outline-none focus:border-[#0528d6] dark:text-white" placeholder="699 00 00 00" /></div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-800 dark:text-slate-400 uppercase italic ml-1">Départ</label><input type="datetime-local" value={form.startDate} onChange={e => setForm({...form, startDate: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-2xl font-bold text-xs outline-none focus:border-[#0528d6] dark:text-white" /></div>
                    <div className="space-y-1.5"><label className="text-[10px] font-black text-slate-800 dark:text-slate-400 uppercase italic ml-1">Retour</label><input type="datetime-local" value={form.endDate} onChange={e => setForm({...form, endDate: e.target.value})} className="w-full p-4 bg-slate-50 dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-2xl font-bold text-xs outline-none focus:border-[#0528d6] dark:text-white" /></div>
                  </div>
                </div>
                <button onClick={() => form.clientPhone ? setStep(2) : setError("Un numéro de téléphone est requis.")} className="w-full py-5 bg-[#0528d6] text-white rounded-[2rem] font-black text-xs uppercase shadow-xl flex items-center justify-center gap-2 italic transition-all hover:bg-blue-700">Continuer <ChevronRight size={18}/></button>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-8 animate-in slide-in-from-right-4">
                <div className="flex justify-between items-center">
                    <h3 className="text-3xl font-[900] italic tracking-tighter uppercase leading-none text-[#0528d6]">Chauffeur</h3>
                    <span className="px-4 py-2 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase italic tracking-widest">{getDurationLabel()}</span>
                </div>
                  ...
                  {drivers.map(d => (
                    <div key={d.id} onClick={() => setForm({...form, driverId: d.id})} 
                       ...
                       <div className="overflow-hidden">
                          ...
                          <p className="text-[10px] font-black text-[#0528d6] uppercase mt-2">
                ...
                <div className="flex gap-4">
                  <button onClick={() => setStep(1)} className="flex-1 py-4 bg-slate-100 dark:bg-slate-800 text-slate-500 rounded-2xl font-black text-xs uppercase italic transition-colors">Retour</button>
                  <button onClick={handleCalculate} disabled={loading || (isDriverRequired && !form.driverId)} className="flex-[2] py-4 bg-[#0528d6] text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl flex items-center justify-center gap-2 disabled:opacity-50">
            ...
            {step === 3 && initRes && (
              <div className="space-y-8 animate-in slide-in-from-right-4">
                <h3 className="text-3xl font-[900] italic tracking-tighter uppercase text-[#0528d6]">Facturation</h3>
                <div className="bg-[#0528d6] rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-2xl">
                  <ShieldCheck size={200} className="absolute -bottom-10 -right-10 text-white/5 rotate-12" />
                  <div className="relative z-10 text-center">
                    <p className="text-[10px] font-bold uppercase opacity-60 mb-1 tracking-widest italic">Devis Global Certifié</p>
                    <h4 className="text-5xl font-black italic tracking-tighter leading-none">{initRes.totalAmount?.toLocaleString()} <span className="text-2xl">XAF</span></h4>
                    <div className="mt-8 pt-8 border-t border-white/10 grid grid-cols-2 gap-6 text-[10px] font-black uppercase italic">
                ...
                <button onClick={handlePay} disabled={loading} className="w-full py-6 bg-green-600 text-white rounded-[2rem] font-black text-xs uppercase tracking-widest shadow-xl hover:bg-green-700 italic flex items-center justify-center gap-2">
                  {loading ? <Loader2 className="animate-spin size-4" /> : "Payer & Valider la Réservation"}
                </button>
                <button onClick={() => setStep(2)} className="w-full text-xs font-black text-slate-400 uppercase italic hover:text-[#0528d6] transition-colors text-center">Modifier mes options</button>
              </div>
            )}
