...
import {
  ...
  Download, LogOut, X, ChevronRight, Activity, Gift,
} from 'lucide-react';
...
export const Sidebar = ({
  ...
  <aside className={`${sidebarOpen ? 'fixed inset-0 z-[200]' : 'hidden'} lg:sticky lg:top-0 lg:flex lg:w-72 flex-col h-screen shrink-0 transition-all duration-300 bg-slate-50 border-r-2 border-slate-200 dark:bg-[#080b14] dark:border-slate-800 shadow-xl`}>
    ...
    <div className="relative z-10 h-full flex flex-col overflow-hidden">
      <div className="flex-shrink-0 flex items-center justify-between py-10 px-6 mb-2">
        <button type="button" className="flex items-center gap-3 group cursor-pointer" onClick={() => { setCurrentView('HOME'); setSidebarOpen(false); }}>
          ...
          <div className="flex flex-col text-left">
            ...
            <span className="text-[10px] font-medium text-slate-500 dark:text-slate-400 mt-1 italic uppercase tracking-widest">
              {t?.sidebar?.systemSubtitle ?? 'Espace client'}
            </span>
          </div>
        </button>
        <button type="button" onClick={() => setSidebarOpen(false)} className="lg:hidden p-2 text-slate-400 hover:text-red-500"><X size={20} /></button>
      </div>
      ...
      <nav className="flex-1 overflow-y-auto no-scrollbar px-4 space-y-8 pb-8 text-left">
        <div>
          <p className="text-[11px] font-black uppercase tracking-[0.2em] text-slate-400 mb-4 px-2 italic">
            {t?.sidebar?.hub ?? 'Navigation'}
          </p>
          <div className="space-y-1">
            <SidebarItem icon={<Home size={20} />} label={t?.sidebar?.home ?? 'Accueil'} active={currentView === 'HOME'} onClick={() => { setCurrentView('HOME'); setSidebarOpen(false); }} />
            <SidebarItem icon={<Store size={20} />} label={t?.sidebar?.marketplace ?? 'Marketplace'} active={currentView === 'CATALOG'} onClick={() => { setCurrentView('CATALOG'); setSidebarOpen(false); }} />
            {isAuth && (
              <>
      ...
      <div className="flex-shrink-0 mt-auto p-4 border-t border-slate-200 dark:border-slate-800 space-y-2">
        {handleInstall && (
          ...
          >
            ...
            <span>{t?.sidebar?.install ?? "Installer l'app"}</span>
          </button>
        )}
        {isAuth && (
          <button type="button" onClick={handleLogout} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-500/10 transition-all italic">
            <LogOut size={18} />
        ...
        <div className="mt-4 p-3 bg-white dark:bg-slate-900/50 rounded-xl border border-slate-200 dark:border-slate-800/50">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[10px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest italic">
                {t?.sidebar?.status ?? 'En ligne'}
              </span>
            </div>
...
const SidebarItem = ({ icon, label, active, onClick, badge }: any) => (
  ...
  >
    ...
    <div className="flex items-center gap-2">
      {badge && (
        <span className={`text-[10px] font-black rounded-full px-2 py-0.5 ${active ? 'bg-white/20 text-white' : 'bg-red-500 text-white'}`}>
          {badge}
        </span>
      )}
