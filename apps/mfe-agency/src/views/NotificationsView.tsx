...
export const NotificationsView = ({ agencyId, t }: { agencyId: string, t: any }) => {
  ...
  const filteredNotifs = useMemo(() => {
    return notifs.filter(n => {
      ...
      const matchesFilter = filterType === 'ALL' ? true : filterType === 'UNREAD' ? !n.isRead : n.reason?.includes('ALERT');
      return matchesSearch && matchesFilter;
    });
  }, [notifs, searchTerm, filterType]);

  if (loading && notifs.length === 0) return <div className="h-96 flex items-center justify-center"><Loader2 className="animate-spin text-[#0528d6]" size={40}/></div>;
...
=======
          ...
          <div className="flex items-center gap-4 mt-2">
            ...
            <span className="text-[10px] font-black uppercase px-3 py-1 bg-blue-50 dark:bg-blue-900/30 text-[#0528d6] rounded-full border border-blue-100 dark:border-blue-800 italic animate-pulse">
      ...
      <div className="flex flex-col lg:flex-row items-center gap-4 bg-white dark:bg-[#1a1d2d] p-4 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="flex bg-slate-100 dark:bg-slate-900 p-1.5 rounded-2xl w-full lg:w-auto shrink-0">
            {['ALL', 'UNREAD', 'ALERTS'].map(id => (
                <button key={id} onClick={() => setFilterType(id as any)}
...
>>>>>>> f89e8e59f63bc1dfac2a96703e19a8ccc0658d21
        ...
        <div className="relative w-full group">
          ...
          <input placeholder={t.header.search} className="w-full pl-12 pr-6 py-3 bg-slate-50 dark:bg-slate-900 border-none rounded-xl text-sm font-bold italic outline-none focus:ring-2 focus:ring-[#0528d6]/20 dark:text-white transition-all" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
      </div>
      ...
      <div className="space-y-4">
        {filteredNotifs.length === 0 ? (
          ...
          filteredNotifs.map((n) => (
            <div key={n.id} className={`p-6 rounded-[2.5rem] border transition-all flex items-start gap-5 ${n.isRead ? 'bg-white/50 dark:bg-[#1a1d2d]/50 border-slate-100 dark:border-slate-800 opacity-60' : 'bg-white dark:bg-[#1a1d2d] border-blue-100 dark:border-blue-900 shadow-md border-l-4 border-l-[#0528d6]'}`}>
              <div className={`size-12 rounded-2xl flex items-center justify-center shrink-0 ${n.reason?.includes('ALERT') ? 'bg-red-50 text-red-500' : 'bg-blue-50 text-[#0528d6]'}`}>
