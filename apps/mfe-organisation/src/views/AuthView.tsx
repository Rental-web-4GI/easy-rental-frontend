...
export const AuthView = ({ onAuth, t, lang, setLang, darkMode, toggleTheme }: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!success) {
      setError("Identifiants incorrects ou quota atteint.");
      setLoading(false);
    }
  };

  return (
    // On utilise justify-center seulement si le contenu est plus petit que l'écran
    // Sinon, on laisse le scroll naturel du parent avec un padding
    <div className="min-h-screen bg-[#f4f7fe] dark:bg-[#080b14] flex flex-col justify-center items-center p-2 md:p-4 transition-colors duration-500 relative">
      ...
      {/* LA CARTE : Hauteur fixe sur Desktop (h-[750px]) pour éviter tout mouvement */}
      <div className="w-full max-w-[1100px] h-full max-h-[730px] min-h-[600px] grid lg:grid-cols-2 bg-white dark:bg-[#1a1d2d] rounded-[2.5rem] shadow-2xl border border-white dark:border-slate-800 overflow-hidden relative z-10 animate-in fade-in zoom-in duration-500">
        ...
        {/* SECTION GAUCHE : Fixe et Immersive */}
        <div className="hidden lg:flex flex-col justify-between p-16 bg-[#0528d6] relative overflow-hidden text-white h-full">
          ...
          <div className="relative z-10">
            ...
            <h2 className="text-5xl font-bold tracking-tighter leading-[1.1] mt-12">
               La mobilité numérique <br />
               <span className="text-blue-300">sans frontières.</span>
            </h2>
          </div>
        ...
        {/* SECTION DROITE : Formulaire stable avec son propre scroll */}
        <div className="flex flex-col h-full overflow-hidden bg-white dark:bg-[#1a1d2d]">
          {/* Scroll interne uniquement pour le formulaire */}
            ...
            <div className="max-w-md mx-auto h-full flex flex-col justify-center">
              ...
              <div className="mb-10 text-center lg:text-left shrink-0">
                <h3 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">
                  {isSignUp ? "Créer un espace" : "Accès manager"}
                </h3>
                <p className="text-slate-400 text-sm mt-2 font-medium">
                  {isSignUp ? "Lancez votre réseau." : "Ravi de vous revoir."}
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                {isSignUp && (
                  <div className="animate-in fade-in slide-in-from-top-4 duration-500 space-y-5">
                    ...
                    <AuthInput label="Organisation" icon={<Building/>} placeholder="Global Rent Ltd" value={form.orgName} onChange={v => setForm({...form, orgName: v})} />
                  </div>
                )}
                
                <AuthInput label="Email professionnel" icon={<Mail/>} type="email" placeholder="manager@rental.com" value={form.email} onChange={v => setForm({...form, email: v})} />
                ...
                >
                  {loading ? <Loader2 className="animate-spin size-5" /> : (
                    <>
                      {isSignUp ? "Lancer l'organisation" : "Accéder au hub"}
                      <ArrowRight size={18} />
                    </>
                  )}
                </button>
              ...
              <div className="mt-10 flex flex-col items-center gap-6 shrink-0">
                ...
                >
                  {isSignUp ? "Déjà manager ? Connectez-vous" : "Pas encore de compte ? Créez une organisation"}
                </button>
