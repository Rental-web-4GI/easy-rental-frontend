      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 px-2">
        <div>
          ...
          <p className="text-[9px] text-slate-400 font-bold italic mt-2">
            Historique conservé en base — vos lectures ici sont indépendantes des consoles agence.
          </p>
        </div>
        <button
          onClick={() => loadNotifs(false)}
          className="p-3 bg-white dark:bg-slate-800 rounded-2xl shadow-sm hover:text-[#0528d6] transition-all border border-slate-100 dark:border-slate-800"
        >
          <Clock size={20} />
        </button>
      </div>
        ...
        <div className="flex bg-slate-100 dark:bg-slate-900 p-1.5 rounded-2xl w-full lg:w-auto shrink-0">
          {[
            ...
            <button
              ...
              className={`flex-1 lg:flex-none px-6 py-2 rounded-xl text-[10px] font-black uppercase italic transition-all ${filterType === tab.id ? 'bg-white dark:bg-slate-800 text-[#0528d6] shadow-md' : 'text-slate-400 hover:text-slate-600'}`}
            >
              {tab.label}
            </button>
          ))}
                  ...
                  <div className="flex items-center gap-4 flex-wrap">
                    {read ? (
                      <span className="text-[9px] font-black uppercase px-3 py-1 bg-green-50 text-green-600 rounded-full border border-green-100 italic flex items-center gap-1">
                        <Check size={10} /> Lu (organisation)
                      </span>
                    ) : (
                      <button
                        onClick={() => handleMarkRead(n.id)}
                        className="text-[9px] font-black text-[#0528d6] flex items-center gap-1.5 hover:underline px-3 py-1 bg-blue-50 rounded-full italic"
                      >
